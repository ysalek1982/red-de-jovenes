    ...
    <section className="min-h-screen bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 px-4 pb-32 pt-32 text-white">
      <div className="section-shell">
        <div>
          ...
          <h1 className="mt-2 text-4xl font-black">Conversaciones que edifican</h1>
        ...
        <div className="mt-8 grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
          <aside className="space-y-5">
            <article className="rounded-[2rem] border border-white/10 bg-white/[0.07] p-5">
              <h2 className="font-black">Mis conversaciones</h2>
              <div className="mt-4 space-y-2">
                {isLoading ? <p className="text-sm text-white/60">Cargando...</p> : null}
                {!isLoading && !conversations.length ? <p className="text-sm text-white/60">Empieza una conversacion que edifique.</p> : null}
                {conversations.map((conversation) => (
                  <button key={conversation.id} type="button" onClick={() => setSelectedId(conversation.id)} className={`w-full rounded-2xl border p-3 text-left text-sm font-bold ${selectedId === conversation.id ? 'border-amber-300/30 bg-amber-300/10' : 'border-white/10 bg-slate-950/45'}`}>
            ...
            <article className="rounded-[2rem] border border-white/10 bg-white/[0.07] p-5">
              ...
              <div className="mt-4 space-y-2">
                {profiles.slice(0, 6).map((profile) => (
                  <button key={profile.id} type="button" onClick={() => void handleCreateDirect(profile.id)} className="flex w-full items-center justify-between rounded-2xl border border-white/10 bg-slate-950/45 p-3 text-left">
                    <span>
          ...
          <article className="rounded-[2rem] border border-white/10 bg-white/[0.07] p-6 shadow-2xl shadow-black/25 backdrop-blur">
            ...
            <div className="mt-5 min-h-72 space-y-3 rounded-3xl border border-white/10 bg-slate-950/45 p-4">
              {!selected ? <p className="text-sm text-white/60">Selecciona o crea una conversacion.</p> : null}
              {messages.map((message) => (
                <div key={message.id} className={`max-w-[85%] rounded-3xl p-4 ${message.sender_id === userId ? 'ml-auto bg-emerald-300/15' : 'bg-white/10'}`}>
            ...
            <form onSubmit={(event) => void handleSend(event)} className="mt-4 flex gap-2">
              <input value={body} onChange={(event) => setBody(event.target.value)} placeholder="Escribe con respeto y animo" className="min-w-0 flex-1 rounded-full border border-white/10 bg-slate-950/70 px-4 py-3 text-sm outline-none focus:border-amber-200" />
              <button type="submit" className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-white text-slate-950"><Send className="h-4 w-4" /></button>
            </form>
          </article>
        </div>
      </div>
