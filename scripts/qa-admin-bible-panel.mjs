...
async function main() {
  ...
  if ((status.data?.translations ?? []).length < 1 || status.data?.booksCount < 66) {
    throw new Error('Panel Biblia no reporta traducciones/libros esperados.')
  }

  const random = await admin.supabase.functions.invoke('admin-ai-settings', {
    body: { action: 'test_random_bible_verse' },
  })
  if (random.error || random.data?.status !== 'BIBLE_RANDOM_OK') {
    throw random.error ?? new Error('Prueba admin de versiculo aleatorio fallo.')
  }

  const denied = await nonAdmin.supabase.functions.invoke('admin-ai-settings', {
    body: { action: 'get_bible_admin_status' },
  })
  if (!denied.error && !denied.data?.error) throw new Error('No admin pudo leer panel Biblia.')

  printOk('QA_ADMIN_BIBLE_OK', {
    translations: status.data.translations.length,
    books: status.data.booksCount,
    nonAdmin: 'DENIED',
  })
}
